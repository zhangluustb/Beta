...
CDllTestModule _Module;

BEGIN_OBJECT_MAP(ObjectMap)
END_OBJECT_MAP()

LONG CDllTestModule::Unlock()
{
	AfxOleUnlockApp();
	return 0;
}

LONG CDllTestModule::Lock()
{
	AfxOleLockApp();
	return 1;
}
LPCTSTR CDllTestModule::FindOneOf(LPCTSTR p1, LPCTSTR p2)
{
	while (*p1 != NULL)
	{
		LPCTSTR p = p2;
...
BOOL CDllTestApp::InitATL()
{
	...
	while (lpszToken != NULL)
	{
		...
		{
			_Module.UpdateRegistryFromResource(IDR_DLLTEST, FALSE);
		...
		{
			_Module.UpdateRegistryFromResource(IDR_DLLTEST, TRUE);
